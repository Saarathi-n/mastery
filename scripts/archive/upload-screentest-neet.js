...
const cloudinaryFolder = 'mastery/neet-screentest';

async function uploadFolder(dirPath, cloudFolder) {
  if (!fs.existsSync(dirPath)) {
    console.log(`Folder not found, skipping: ${dirPath}`);
    return;
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await uploadFolder(fullPath, `${cloudFolder}/${entry.name}`);
    } else if (
      ...
      try {
        ...
        if (!exists) {
          await PDF.create({
            ...
            type: 'screentest', // Use type 'screentest' as done for jee
